...
> [!WARNING]
> **Это не официальный Desktop Mode Android.** Компьютер с Windows не определяется телефоном как физический монитор. Проект показывает отдельный виртуальный дисплей Android через scrcpy; полноценные плавающие окна уровня DeX не гарантируются. Официальный режим внешнего экрана требует прямого подключения Pixel к совместимому монитору или док-станции через USB-C/DisplayPort/HDMI.
